...
export default function GenreCard({ name, cover, id }: GenreCardProps) {
  return (
    <Link to={`/genre/${id}`}>
      <article className='group relative h-48 w-[calc(100vw-2px)] cursor-pointer rounded-sm shadow outline transition-all duration-200 hover:-translate-y-1 hover:outline-2 hover:outline-red-400 sm:w-96'>
